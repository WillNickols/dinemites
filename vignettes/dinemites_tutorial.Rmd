...
if (instantiate::stan_cmdstan_exists()) {
    ...
    probabilities_bayesian_mat <- read.csv(system.file(package = "dinemites",
            "extdata",
            "probabilities_bayesian_mat.tsv"), 
        header = FALSE, 
        sep="\t")
}
```
...
if (instantiate::stan_cmdstan_exists()) {
    ...
    probabilities_clustering_mat <- read.csv(system.file(package = "dinemites",
            "extdata",
            "probabilities_clustering_mat.tsv"), 
        header = FALSE, 
        sep="\t")
}
```
...
With probabilities assigned to each allele observation, we can now compute
the new complexity of infection (COI) on each day: the number of unique genetic
variants (i.e., parasite variants) new to each day. When only a single locus
is used, we simply sum the probabilities the alleles are new at each day
to get the new COI. When there are multiple loci, adding across the loci would
